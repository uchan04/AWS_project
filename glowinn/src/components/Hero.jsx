...
// The asset is committed with the project and served from our own hosting — the
// template library's storage URL never appears anywhere in this repo.
    ...
    <section className="hero" id="top">
      ...
      <div className="hero__foot shell">
        <article className="card card--note">
          ...
          <p>
            From radiant skincare to flawless makeup, explore products designed for every skin type
            and style.
          </p>
        </article>
        ...
        <div className="hero__stats">
          {STATS.map((stat) => (
            <article key={stat.figure} className="card card--stat">
              <strong>{stat.figure}</strong>
              <span className="card__label">{stat.label}</span>
              <span className="card__foot">{stat.foot}</span>
            </article>
          ))}
        </div>
      </div>
