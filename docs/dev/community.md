...
- 진행 중: 없음
- 미착수: 이미지 업로드 (`SPEC.md` 12절 제외 항목. `_lib/imageKey.ts`의 소유 검증만 있고 presign 경로가 없다)
- **2026-08-25 정정: LLM 주제 추천은 구현됐다** — `GET /api/community/topics` + `lib/community/topics.ts`(Bedrock). `_lib/topics.ts`의 고정 문구는 **호출 실패 시 대비책**으로 남은 것이고 대체가 아니다. 이 줄이 "미착수"에 남아 있었다
