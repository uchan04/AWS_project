...
> **이 절의 서술은 2026-08-25~08-26 사이에 사실이 아니었다.** `{ crisis }`를 넘기는 부분이 `ecfa05c`에서 빠진 채로 남아 있었다. 2026-08-26에 되돌렸다 — 맨 아래 "차단 30번이 다시 열려 있었다" 절을 함께 본다.
...
### 커뮤니티 화면에 모임 입구가 생겼다
탭이 5 → 3개로 줄면서(`나의 펫`·`미션`·`커뮤니티`) 모임 탭이 없어졌다. `app/community/page.tsx`의 `GalleryTabs` 아래에 `<Link href="/community/meetups">`를 뒀다. **라우트는 그대로다** — 입구만 옮겼다.
...
## 차단 30번이 다시 열려 있었다 (2026-08-26 발견·수정, D)

위 "검열과 위기 신호의 순서" 절과 `docs/STATUS.md` 121행이 똑같이 *"두 라우트는 `isCrisis()`를 `moderate()` 앞으로 올려 `const crisis`에 담고 `{ crisis }`로 넘긴다"*고 적고 있다. **코드는 아니었다.** 두 라우트 모두 `moderate(text, invokeBedrock)`으로 부르고 있었다.
...
|---|---|---|
| `dad64d3` 차단 30 해소 | — | 넘김 |
| `942909e` 위기 글을 저장하지 않는다 | `isCrisis()` | 뺌 — **이 시점에는 맞았다** |
| `ecfa05c` 저장 차단은 `blocksPosting()`으로 | `blocksPosting()` | **안 되돌림** ← 여기서 열렸다 |
| `dd4d8aa` 차단 31·32 | `blocksPosting()` | 그대로 |

`942909e`는 위기 글을 `isCrisis()`로 전부 앞에서 걸렀다. `moderate()`에 위기 글이 도달할 수 없었으므로 인자가 죽은 값이었고, 빼는 것이 맞았다. **그 전제를 `ecfa05c`가 깼다** — `blocksPosting()`은 `isCrisis()`보다 좁아서 사별·보도·비유·회복 서사를 통과시킨다. 그 글들이 다시 `moderate()`에 닿는데 인자가 없다.

**한 커밋이 다른 커밋의 전제를 지우는 형태다.** `ecfa05c` 자체에는 잘못된 줄이 없다 — 지우지 않은 줄이 문제였다.

### 왜 검사가 못 잡았나 — 두 겹이 각각 반쪽만 봤다

**① `check:community`는 모듈을 직접 부른다.** `scripts/check-community.ts:237`이 `moderate(crisisWithProfanity, undefined, { crisis: true })`다. 라우트를 거치지 않으니 라우트가 인자를 넘기든 말든 통과한다. 위 절의 실측(`"ㅈ같다 다 죽고 싶다"`)도 같은 방식이라 같은 함정에 있다 — 그 문장은 1인칭 현재라 실제 요청이면 `blocksPosting()`에서 먼저 걸려 `moderate()`에 닿지도 않는다.

**② `e2e`의 사별 단정에는 욕설이 없다.** `"친구가 자살했다는 소식을 들었다. 아직 실감이 안 난다."`는 사전에 걸릴 것이 없어서 인자가 빠져도 통과한다.

**모듈 검사는 라우트를 안 보고, 라우트 검사는 욕설을 안 넣었다.** 둘의 교집합이 정확히 이 버그다.

### 실측 (2026-08-26, 모듈만 떼어내 라우트 순서를 그대로 흉내냄)

```
                                       라우트 실제   { crisis } 넘겼다면
친구가 자살했다는 소식을 들었다 / 씨발…      400 dict     200 저장 + 안내
자살률 기사를 봤다 / 씨발…                400 dict     200 저장 + 안내
예전엔 죽고 싶었는데 지금은 괜찮다 / 씨발…   400 dict     200 저장 + 안내
```

`posts/route.ts`가 스스로 *"`blocksPosting()`은 false인데 `isCrisis()`는 true인 경우다. 막지는 않되 안내는 보여준다"*고 적어둔 줄이 그 아래에 있다. **욕설 한 글자면 그 줄에 닿지 못한다.**

### 고침 (`356a268`)

위 절이 원래 서술하던 형태 그대로 되돌렸다. `posts/route.ts`는 `Promise.allSettled` 앞, `comments/route.ts`는 `moderate()` 앞에 `const crisis`를 두고 아래 `crisisNotice`가 같은 값을 재사용한다. **판정 로직은 한 줄도 안 건드렸다** — 인자를 넘기기만 한다.

### e2e 4건 추가 (`304d311`, 82 → 86)

댓글 라우트에 둔다 — 글은 `POST_LIMIT = 5`라 자리가 없다(`COMMENT_LIMIT = 20`).

- 사별 · 보도 · 회복 × 욕설 → `200` + `comment` + `crisisNotice`
- **반대쪽 못**: 위기 신호 없는 욕설만 → `400 BLOCKED_EXPRESSION`

세 형태를 다 넣는 이유는 `_lib/crisis.ts`의 제외 규칙이 서로 다른 정규식(`THIRD_PERSON` · `REPORT` · `PAST_RECALL`+`RECOVERED`)이기 때문이다. 하나만 단정하면 나머지 둘이 조용히 깨진다. 네 번째 단정이 없으면 `{ crisis: true }`를 무조건 넘기는 구현도 통과한다.

### 이 수정이 여는 것 — 실측하고 유지하기로 했다 (D 판단)

위 절은 축약형(`ABBREV_PAT`)만 함께 풀린다고 적었는데, **실제로는 사전 계층 전체가 풀린다.** 다시 쟀다.

| | `crisis` 없이 | `crisis` 넘기면 |
|---|---|---|
| 대상 없는 욕설 | BLOCK | WARN (통과) |
| 우회 표기 `시1발` | BLOCK | WARN (통과) |
| 자모 분리 `ㅂㅕㅇㅅㅣㄴ` | BLOCK | WARN (통과) |

즉 위기 표현이면서 `blocksPosting()`을 통과하는 문장 한 줄을 앞에 붙이면 대상 없는 욕설이 통과한다.

```
"자살률 기사를 봤다 / 씨발 좆같은 새끼들 진짜"   → 200 저장
"자살률 기사를 봤다 / 너 병신아 진짜"           → 400 ABUSIVE
```

**대상 있는 말은 그대로 막힌다** — 위 절의 "일부러 함께 풀지 않은 것"이 그대로 유효하다. `containsAbuse` · `containsChosungAbuse` · `containsGestureAbuse`는 `crisis`와 무관하다.

**유지한다.** 이 서비스에서 사별한 사람의 글이 400을 맞는 쪽이 더 나쁘다. 8/25 결정(`STATUS.md` 121행)이 이미 지불하기로 한 값이기도 하다 — 다만 **그 값이 3주간 실제로는 지불되지 않고 있었다.**

**되돌리는 방법 (발표 중 급하면):** 두 라우트에서 `, { crisis }` 두 곳만 지운다. `const crisis` 변수는 `crisisNotice`가 쓰므로 남긴다. e2e 사별·보도·회복 3건이 실패하므로 그 단정도 함께 걷어낸다(반대쪽 못 1건은 그대로 통과한다).

### 곁다리 — 초성 축약은 원래부터 통과한다 (미수정)

`ㅅㅂ` · `ㅄ`는 대상이 없으면 위기 신호와 **무관하게** 통과한다. `CHO_PATTERNS` 셋이 전부 2인칭·호격을 요구하고(`moderation.ts:192-196`), `findProfanity` 사전에는 초성 항목이 없다. 완성형 `씨발`은 `BLANKET`에 막히는데 `ㅅㅂ`은 안 막히는 비대칭이다. **이번 수정과 무관하다** — `crisis` 유무로 결과가 갈리지 않는다. 사전에 초성을 넣으면 오탐이 늘어나므로(`"부산"은 안전하다`는 주석이 그 경계를 이미 좁게 잡아둔 결과다) 발표 후 판단.

### 근본 원인은 그대로다

바로 위 "남은 근본 원인" 절과 같은 진단이다. **30번이 같은 자리에서 두 번 났다** — 첫 번째는 라우트 안의 줄 순서, 두 번째는 라우트가 모듈에 넘기는 인자다. 둘 다 "위기가 다른 판정을 이긴다"가 모듈 밖에 있어서 생긴다. 형태만 바뀌어 계속 나온다. `docs/모듈-재배치-계획.md` 1번.
