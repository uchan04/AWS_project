...
**아직 `npm run db:seed`를 돌리지 않았다.** 코드상 배치는 바뀌었지만 공유 DB의
`Mission` 909행은 옛 배치 그대로다. 시드를 돌려야 화면에 반영된다.

돌리기 전에 확인할 것: 미션 code 규칙이 `{유형}_S{단계}_{슬롯}`이라 단계·슬롯이 같으면
같은 행에 덮어써진다. 그런데 **이 변경은 어느 슬롯에 어느 미션이 오는지를 바꾸므로**
`UserMission` 241행이 가리키는 미션의 제목이 달라진다 — 이미 완료한 기록의 이름이
바뀐다는 뜻이다. 보상이나 완료 여부가 어긋나는 것은 아니다.

마이그레이션은 아니지만 공유 DB 쓰기다. 혼자 돌리지 말고 팀에 알린 뒤 한다.
