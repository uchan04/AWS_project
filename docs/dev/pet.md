...
- 완료: 방치형 자동 획득 (`6d942ce`), 치장 착용·해제 (`ea9f61c`), 친밀도 캐릭터 구매·전환 (`025d09d`)
- **SPEC.md 5절의 기능은 전부 구현했다.** 남은 것은 팀 결정 대기 1건(치장 획득 경로)과 런타임 검증이다
...
- `app/api/pet/cosmetics/route.ts` — GET 목록·진행률 / POST 착용·해제
- `app/api/pet/skins/route.ts` — GET 캐릭터 목록
- `app/api/pet/skins/buy/route.ts` — POST 구매 (친밀도 300)
...
- 치장 획득 경로 미정. 가챠를 삭제해서 경로가 사라졌고 별조각도 소모처가 없다 (`SPEC.md` 5절에 팀 논의 필요로 적혀 있다). **착용 화면(`ea9f61c`)까지 다 만들어 놨으므로 이제 이 결정만 남았다** — 경로가 없으면 화면이 늘 "미획득"이다
...
**기능 구현은 끝났다.** 남은 것은 공유 DB 쓰기 승인이 필요한 3건과 팀 결정 1건이다.
...
3. **런타임 검증** — 아래 네 흐름을 브라우저에서 한 번씩 돌린다. 전부 검증용 재화가 필요해 **공유 DB 쓰기 승인이 있어야 한다.**
   - 씨앗 투입 → 레벨업 → 진화 연출 (유저 `seeds`가 0이라 씨앗 주입 필요)
   - 방치형 수령 (`lastIdleClaimAt`을 과거로 밀어 두면 즉시 확인된다)
   - 치장 착용·해제 (`UserCosmetic`이 0행이라 보유 행 주입 필요)
   - 캐릭터 구매·전환 (`affinity`가 0이라 친밀도 주입 필요)
4. **치장 획득 경로·별조각 소모처 결정** (팀). 가챠 삭제로 둘 다 도달 불가가 됐다. **착용 화면은 다 만들었지만 획득 경로가 없어 늘 "미획득"으로 보인다.** 발표에서 이 화면을 쓸 거라면 이 결정이 먼저다

**E에게 요청할 것**: `GachaPull`·`heroPity`·`legendPity` DROP 마이그레이션 1개 (`docs/STATUS.md` 차단 6번), `npm run db:seed`의 `.env` 로딩 (차단 7번)
**A에게 알릴 것**: `SEED_TO_EXP` 1 → 10 변경(`lib/types.ts`), `app/page.tsx:37` lint 에러. 홈 화면에 쓸 수 있는 값으로 `GET /api/pet`이 `idle.seeds`(미수령 방치형 씨앗)를 돌려준다
