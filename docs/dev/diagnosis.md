...
## 23. 홈 제거와 진단 직후 알 팝업 (2026-08-26, A. 사용자 지시)

### 홈이 화면에서 갈림길이 됐다

`app/page.tsx` **101줄 → 65줄**. 분기가 셋뿐이다.

```
미인증        <Intro authed={false} />
진단 미완료    <Intro authed />
진단 완료      redirect("/pet")
```

`HomeDashboard`·`loadMissions`·스킨/미션 조회를 걷었다. **홈 카드 3장의 고유 정보가 0이었다** — 실측 근거는 `docs/dev/pet.md` "화면 구성 판단"에 있다.

| 홈 카드 | 같은 값이 어디에 또 있었나 |
|---|---|
| `오늘의 나` | 미션 탭 `ProgressCard` 3칸이 **정확히 같은 세 값** |
| `오늘의 미션` | 미션 탭 일일 미션 목록 |
| `키우기` | 사이드바 `나의 펫` 탭 |

`app/components/Sidebar.tsx`의 `TABS`가 5 → **3**(`나의 펫`·`미션`·`커뮤니티`)이 됐다. **모임은 커뮤니티 화면 상단 입구로 내렸다** — 탭 하나를 쓸 사용 빈도가 아니다. 라우트(`/community/meetups`)는 그대로다.

### 진단 직후 "알을 받았어요" 팝업

`app/diagnosis/result/page.tsx`. **`?new=1`일 때만 뜬다** — 결과 화면을 다시 방문할 때는 뜨지 않는다(같은 파일의 재방문 문구 규칙과 같은 조건이다).

전에는 알을 받고 나서 *이게 뭐가 되는지* 모른 채 시작했다. 진화 4단계를 보여주는 자리가 `/pet`의 🌟 버튼 하나였고, 그건 이미 시작한 사람만 누른다.

```
🥚 알을 하나 받았어요
   고양이 알이에요. 미션을 하면 씨앗이 모이고, 그 씨앗으로 자라요.
...
- 구간은 `EVOLUTION_LEVEL`에서 읽는다 — 손으로 적으면 값이 바뀔 때 조용히 거짓이 된다. `/pet` 진화 팝업과 **같은 값**이다
- **그림을 넣지 않았다.** 이 화면은 스킨을 모른다(종족만 안다). 없는 그림 자리를 만들지 않는다
- **잠금 표시(🔒)를 쓰지 않았다.** 결핍 목록이 아니라 열망을 만드는 자리다
- **`지금 미션 하러 가기` 같은 재촉 버튼도 두지 않았다.** 진단을 막 끝낸 사람에게 다음 과제를 바로 얹지 않는다
- 접근성은 `useModalA11y(onClose, open, initialFocus)` — 이 앱의 모달 훅 하나를 그대로 쓴다(Escape·초점 덫·초점 복귀·본문 스크롤 잠금)

스타일은 `styles/tokens.css`의 `.hm-egg*`다. **오버레이 값(색·z-index 400·반경·그림자)은 Sidebar의 내 계정 모달과 같다** — 팝업이 두 종류로 보이지 않게 한다.

### 실측

새 계정 `egg-1787715501474@welli.local`(`INDEPENDENT_LOW_INCOME`)으로 확인했다. 제목·안내문·4단계 구간·종족색 "지금" 배지가 전부 나온다. **이 계정은 심사 전 삭제 대상이다**(`npx tsx -r dotenv/config scripts/list-test-rows.ts`로 센다).
