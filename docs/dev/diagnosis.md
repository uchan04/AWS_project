...
**색·폰트·모서리·모션 값은 Figma 프로토타입에서 가져왔다** (`isol-design_Figma/README.md`
"디자인 규칙" 절). 구조는 그대로 두고 값만 바꿨다. 옮긴 것과 옮기지 않은 것:
...
| `fadeSlideIn`·`float`, `.card-hover` 그림자 | 인라인 style 객체 방식 |
...
**Figma 값 반영분은 브라우저 클릭 확인을 다시 하지 못했다.** 다른 세션이 3000 포트의 dev
서버를 잡고 있고 Next는 같은 폴더에서 dev 서버 두 개를 띄우지 못한다. 대신 실행 중인 서버가
내보내는 CSS와 HTML을 직접 받아 팔레트·폰트·새 클래스가 들어간 것을 확인했고
`npm run build`·`npm run check:diagnosis`는 통과한다. 플로우 자체는 값 교체 전에 확인해 뒀다
(첫 선택지 경로 9문항, 두 번째 선택지 경로 12문항). 포트가 비면 세 화면을 눈으로 다시 본다.
