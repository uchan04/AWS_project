...
2026-08-24 추가: C가 **펫 그림을 2배로 키웠다**(사용자 요청 "펫 2배로 키워줘"). 단계별 6.25/8/9.75/11.5rem → **12.5/16/19.5/23rem**(단일 형태 기본값 9 → 18rem). 단계별로 다른 비율은 주지 않았다 — 8/23에 머리 위 Lv 배지가 걷힌 뒤로 단계 간 크기 차이가 "자랐다"는 유일한 신호다. **값을 2배 하는 것보다 값이 흩어져 있는 것이 문제였다** — 크기를 읽는 곳이 그림·발밑 그림자·말풍선 자리 세 군데였고 각자 자기 숫자를 갖고 있어서 8/22에는 그림만, 8/23에는 방만 커졌다. `.pet-room`에 `--pet-char-size`(단계별 네 줄) → `--pet-char-w`(상한 한 번) 한 줄기를 세우고 세 곳이 그것만 읽게 했다(발밑 그림자 0.44배, 이모지 폴백 0.7배, 말풍선 자리 `그림 + 0.75rem` — 셋 다 손으로 맞춰 온 옛 값들의 비율 그대로다). **딸려 온 두 가지는 남겨 뒀다:** ① **좁은 화면은 2배가 아니라 1.48배다** — 방이 19.5rem(312px)이라 4단 23rem이 잘려서 `min(…, 78vw, 17rem)` 상한을 붙였다. 17rem은 좁은 방의 87%로 넓은 화면의 85%(23/27)와 같은 비율이고, 온전히 2배를 원하면 **방의 `min-height`를 먼저 올려야 한다.** ② **말풍선이 그림 상자 위쪽 투명 여백으로 48px 내려앉는다** — 8/24 전까지 한 번도 안 걸렸던 방어 상한(방 높이 − 말풍선 높이)에 넓은 화면 3·4단과 좁은 화면 전부가 걸렸다. 상한을 걷으면 말풍선이 방 밖으로 잘리고, 방을 대사 때문에 키우는 쪽은 8/23에 배제했다(닫는 순간 높이가 튄다). 꼬리가 캐릭터 머리에 닿기 시작하면 방을 키우는 것이 답이다. `pet.css`만 변경, 새 색·새 DOM 0개. `tsc` 0오류 · `check:pet` 통과 · `build` 통과 · 개발 서버가 보내는 CSS 청크에서 여섯 규칙 실측. 상세는 `docs/dev/pet.md` "펫 그림을 2배로"
