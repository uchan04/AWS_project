...
2026-08-24 추가: C가 **상점 두 화면을 Figma Make 시안(`상점 디자인 수정 요청`)의 구성으로 다시 짰다** — 전면 배너(장식 이모지 4개 + `✦ APPEARANCE SHOP ✦` 알약) · 착용 중 카드 한 장 · 🌼 구분선 · **탭 3개(전체 / 보유 중 / 구매 가능)** · 등급 배지와 보유·착용 코너 배지 · 빈 상태 · 끝 장식(🍀✦🌸✦🍀). **색은 시안에서 옮기지 않았다(사용자 결정 3건).** ① 띠 색은 같은 날 두 번 정해졌다 — 먼저 펫 배경색으로 옮겼고(띠 `--color-paper-2` / 화면 `--color-paper`), 로컬에서 화면을 본 사용자가 **"위에 나무 무늬 아예 진갈색으로 바꿔줘"**로 뒤집어 **띠가 `--color-ink-2`(#4A3B29)**가 됐다(한 단 차이로는 배너로 읽히지 않았다). 시안의 나무 3색이 아니라 **있는 잉크 보조색**이라 **새 색은 0개**고 8/21에 지운 `--pet-wood*`도 되살아나지 않았다. 면이 어두워지면서 제목·안내문·눈썹 알약이 크림으로 뒤집혔다(8/23 레벨 배지와 같은 규칙 — 면을 정하면 글자색도 정해진다). 흰 알약 2개(잔액·펫으로)는 그대로다. ② 등급 3색(초록·파랑·보라)은 종족색 진하기로 — 일반은 캔버스색, 희귀 이상은 `--tribe-chip`이고 **에픽 전용 색은 만들지 않았다**(시드에 없다). ③ 두 화면 모두 적용. 시안의 토스트는 안 넣었다 — 상점 안내는 다음 동작까지 남아야 하므로 기존 `.pet-msg`를 유지했다. 배경 상점은 슬롯 3섹션을 탭 하나로 접었다(배경 6종뿐이라 두 섹션이 늘 비어 있었다. 슬롯은 타일 메타 `🏞️ 배경`으로 남는다). 나무결 6줄은 `repeating-linear-gradient` 한 줄로 줄여 DOM이 6개 줄었다. `pet.css` + 목록 2개 + 오류 폴백 2개만 변경, 스키마·API 무수정. `tsc` 0오류 · `check:pet` 통과 · `build` 통과. **로컬 실행으로 세 화면 200과 타일 배지 갈림·수집 게이지·이미지 3장까지 실측했다**(착용 중인 배경이 없는 계정이라 배경 상점의 착용 중 카드는 의도대로 빠졌다). 상세는 `docs/dev/pet.md`
