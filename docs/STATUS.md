...
| E | 인프라 + 인증 | RDS·Cognito·S3+CloudFront·CloudWatch+SNS·Bedrock·auth 실검증·하단 탭 내비 완료, PR #1 머지 + 2차 마이그레이션 + auth 빌드 수정 + 색 토큰 정리 완료. 로그인 화면(이메일+비밀번호/Google) + 쿠키 기반 인증 전환 + `amplify.yml` 완료(2026-08-20). **2026-08-24: 배포 환경 리다이렉트가 `localhost:3000`으로 튀던 버그 수정**(`fd8c21f`, 배포 후 실응답 확인). Amplify GitHub 연동·Google IdP 연결은 완료 확인됨 **2026-08-24: 커뮤니티 글 이미지 업로드 경로(D 요청)** — `POST /api/upload/community/presign` + `lib/uploads.ts` + `npm run check:uploads`. `community/{userId}/{랜덤16}.{ext}`, 5MB·jpg·png·webp. 미션용 `lib/missions/upload.ts`는 무수정(상수가 `verifyS3Object()`와 공유돼 함께 느슨해진다) | Google 로그인 브라우저 실사용 검증만 남음. 세부는 `docs/dev/infra.md` "배포 환경에서 절대 URL을 만들지 않는다"·"커뮤니티 글 이미지 업로드". **D에게 넘긴 계약**: 저장 전 `isOwnCommunityKey()`·`verifyCommunityObject()` 필수, 표시는 `cdnUrl()`. **팀에 알림**: 버킷 정책이 `/*` 전체 공개라 `missions/`의 미션 사진도 URL만 알면 읽힌다(prefix 단위 접근 제어 없음 — 인프라 결정 필요) |
...
- **사진 미션(B)은 로컬에서만 업로드가 안 된다.** 로컬 `.env`의 `S3_BUCKET`이 빈 값이다. 비전 모델은 `lib/missions/vision.ts:7`이 `us.amazon.nova-2-lite-v1:0`으로 폴백하므로 `BEDROCK_VISION_MODEL_ID`는 비어도 된다. **2026-08-24 정정(E): 배포본은 막혀 있지 않다** — Amplify 환경변수 12개를 CLI로 실측했고 `S3_BUCKET=welli-uploads-185236887369`·`CLOUDFRONT_DOMAIN=https://diros91hbap9v.cloudfront.net`·`BEDROCK_VISION_MODEL_ID=amazon.nova-2-lite-v1:0`·`DEV_AUTH_BYPASS=false`가 전부 들어 있다. 즉 이 절의 "값이 안 왔다"는 **로컬 `.env` 한정**이고, 배포에서 안 되는 것이 있으면 원인은 환경변수가 아니다
...
**27. 배포본의 간헐 500은 RDS 커넥션 고갈이다. `DATABASE_URL`에 `connection_limit=1`을 붙이고 재배포해야 한다 (E 담당)** — "로그인을 비롯한 기능이 랜덤하게 동작하거나 하지 않는다"의 원인이다. CloudWatch 로그 스트림 53개를 전수 조사해 확정했다.
