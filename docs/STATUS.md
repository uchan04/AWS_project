...
최종 갱신: 2026-08-24 (E: **배포 환경 리다이렉트 `localhost:3000` 버그 수정 + 배포·실응답 확인**(`fd8c21f`). `request.url`의 host가 Amplify SSR에서 `localhost:3000`이라 로그아웃·로그인 완료 리다이렉트가 사용자 PC로 튀었다 — 앱 내부 리다이렉트는 `lib/cognito.ts`의 `appRedirect()`로 상대 경로를 쓴다. Google IdP 연결 상태도 CLI로 재확인. **`main` 직접 push가 GitHub 브랜치 보호 규칙("PR 필수")을 우회했다 — 아래 "미확정" 5번과 같은 사안, 팀 결정 필요**. 같은 날 **모꼬지 Figma 시안 에셋 41장을 `public/images/`에 추가**(`feat/infra`, `cea04c9`) — 이미지는 DB에 넣지 않는다, 아래 차단 27번 참고)

이전 갱신: 2026-08-21 (흐름 확정: 소개 → 가입/로그인 → 문항 → 결과 → 홈. 자체 DB 계정 + 쿠키 로그인 유지 완료. D 미션 완료 연동 완료, 차단 17·20·22 해소 + 21의 D 몫 해소. C가 `feat/pet`에 `develop`을 머지하고 `/pet`을 Figma 시안으로 이관 + 배고픔 게이지 + **3단 → 4단 진화** + **공유 DB 반영**(차단 24·25 해소) + **차단 19 해소(북극곰 이미지 키)** + **런타임 3흐름 검증** + **상점 2화면 디자인 이관 — 펫 화면군 3장이 한 어휘가 됐고 C는 남은 코드가 없다** + **`develop` 재머지 후 `develop`에 올림(`4cad58f`)**. 신규 차단 **26번 — `SESSION_SECRET`이 없어 로그인이 500이다**. 남은 차단은 13·14·21·23·26)
...
**27. 모꼬지 Figma 에셋 41장이 `public/images/`에 들어왔다 — 화면 담당자가 쓰면 된다 (E, 2026-08-24, `feat/infra` `cea04c9`)** — 아이콘 6종(`home`·`chat`·`growth`·`profile`·`calendar`·`heart`, 각 78×79), 로고 2종, 말풍선 6종, 장식 11종(`decor_01`~`decor_11`), 캐릭터 아트 11종(여우·고양이·곰 각 3포즈 + `character_group`·`sunset_group`), 식물·간판 5종. 참조는 `/images/<파일명>.png`. **S3가 아니라 `public/`에 둔 이유**: 스키마의 이미지 필드는 전부 S3 키 문자열인데(`PetSkin.imageKeyBase`·`CosmeticItem.imageKey`·`UserMission.photoKey`·`Post.imageKey`) 이 41장에 대응하는 모델이 없다. 정적 UI 에셋이라 `public/`에 두면 `CLOUDFRONT_DOMAIN`이 빈 값인 현재 상태에도 로컬·배포 양쪽에서 그대로 보인다.
...
- **위 19번의 남은 항목(`cosmetics/bg-1.png`~`bg-6.png`, 403)은 이 41장으로 막을 수 없다** — 배경 이미지가 아예 없다. 2026-08-24 `cosmetics/bg-1.png` 403 재확인. 단서 하나: S3에 `backgrounds/forest-autumn-{0,1,2}-{0,1}.png` **6장**이 200으로 이미 있고 각 500KB짜리 배경 그림이다. 개수가 맞으므로 `cosmetics/bg-1..6.png`로 복사하면 닫힐 가능성이 있는데, **어느 파일이 `배경1`인지는 눈으로 봐야 해서 손대지 않았다**
