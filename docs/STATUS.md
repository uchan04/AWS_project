...
| D | 커뮤니티 + 챗봇 | 커뮤니티(목록·상세·좋아요·댓글·글쓰기·삭제) 완료, 챗봇 메시지 API + 패널 UI(`/chat` 개발용 라우트) 완료 | `ChatPanel`을 `layout.tsx` 전역 오버레이로 이전 — E와 조율 필요. Bedrock 실제 호출·LLM 주제 추천·전체 탭 글쓰기·일일 미션 연동은 보류. `app/chat/` 폴더 소유가 `CLAUDE.md` 2절 표에 없어 팀 확인 필요 |
