...
최종 갱신: **2026-08-24 — 100단계 커리큘럼의 복습 배치를 계측했다(A, **미수정·B 판단 대기**). `docs/dev/missions.md` "복습 배치 계측" 절.** 풀과 도입 순서는 잘 짜여 있다 — 간격 반복이 확장 간격(1·2·4·7·12·24·50단계) 형태로 실제로 나오고 구간별 새 미션도 12개로 균일하다. 문제는 `pickReview()`가 **순수 전역 LRU라 난이도를 모른다**는 것 하나다. 그래서 (1) 복습 189슬롯의 **31%가 1구간 출신**이고 — 면접을 다녀오는 94단계에 `기지개 한 번`, 98단계에 `양치하기`가 들어간다 — (2) 반복이 거꾸로다: `커튼 열기` 7회 대 `카페에서 주문하기` 2회이고 `먼저 말 걸어보기`·`고맙다고 말하기` 같은 7~9구간 미션 13개는 **딱 한 번만** 나온다(늦게 도입된 미션은 확장 간격의 세 번째 복습이 100단계를 넘어가서 자리가 없다), (3) **10구간 복습에 7~9구간 출신이 0개**다 — "사회로 한 걸음"을 걷는 구간에서 복습하는 것이 전부 혼자 하는 일이다. 고치는 값은 선택기 한 줄이다(복습 후보를 현재 구간 ±3으로 제한). `check:curriculum`에 넣을 단정 3개도 같이 적어 뒀고 **현재 값은 셋 다 위반**이다. `prisma/seed/curriculum.ts`는 B 소유이고 고치면 `db:seed`로 공유 DB의 `Mission` 909행을 다시 써야 한다.**
