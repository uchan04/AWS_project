...
| 담당 | 범위 | 상태 | 비고 |
|---|---|---|---|
| A | 진단 + 미션 콘텐츠 | 미착수 | 미션 41개가 8/15 마감 |
...
1. **RDS 없음** — `DATABASE_URL`이 비어 있어 `prisma migrate dev`를 아직 실행하지 못했다. `prisma/migrations/`가 없다. E 담당, 8/14 최우선
2. **GitHub 원격 없음** — 4명이 clone할 대상이 아직 없다. E 담당, 8/14 최우선
3. **`lib/auth.ts`가 스텁** — `DEV_AUTH_BYPASS=true`로만 동작한다. 실제 Cognito 검증은 8/15 마감

## 마일스톤

| 날짜 | 목표 | 상태 |
|---|---|---|
| 8/14 | 레포·프로젝트·브랜치, auth 스텁, schema 초안 | 진행 중 |
| 8/15 | schema 확정, 첫 라이브 배포, 미션 41개 | |
