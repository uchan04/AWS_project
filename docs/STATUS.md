...
최종 갱신: 2026-08-24 (E: **배포 환경 리다이렉트 `localhost:3000` 버그 수정 + 배포·실응답 확인**(`fd8c21f`). `request.url`의 host가 Amplify SSR에서 `localhost:3000`이라 로그아웃·로그인 완료 리다이렉트가 사용자 PC로 튀었다 — 앱 내부 리다이렉트는 `lib/cognito.ts`의 `appRedirect()`로 상대 경로를 쓴다. Google IdP 연결 상태도 CLI로 재확인. **`main` 직접 push가 GitHub 브랜치 보호 규칙("PR 필수")을 우회했다 — 아래 "미확정" 5번과 같은 사안, 팀 결정 필요**)
