...
| D | 커뮤니티 + 챗봇 | **`develop`에 미반영**(본인 댓글 삭제, 모달 버그 2건, 문서 2건 + 이후 작업). 코드는 충돌하지 않고 예측 충돌은 `docs/STATUS.md` 1건뿐이다. 기능 구현 끝. 챗봇 Bedrock 스트리밍 연결 완료(2026-08-19). **본인 댓글 삭제 + 모달 액션 실패 인라인 표시 완료(2026-08-20)**. 미션 완료 연동 호출부(`completeMission`)는 주석으로 준비만 해둠 | `completeMission()`(B) 대기 상태. 막힌 항목 5개와 주의사항은 `docs/dev/community.md` 상단 "재개 지점" 참고 |
