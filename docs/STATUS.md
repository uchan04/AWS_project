...
| D | 커뮤니티 + 챗봇 | **`develop`에 미반영**(본인 댓글 삭제, 모달 버그 3건, 챗봇 전역 오버레이 이전, 인증 실패 처리, 글쓰기 주제 추천, 문서). 코드는 충돌하지 않고 예측 충돌은 `docs/STATUS.md` 1건뿐이다. **PR #3(→ `develop`) 열려 있음** — `app/layout.tsx` 2줄 변경이 E 리뷰 대상. 기능 구현 끝. 챗봇 Bedrock 스트리밍 연결 완료(2026-08-19). 미션 완료 연동 호출부(`completeMission`)는 주석으로 준비만 해둠 | `completeMission()`(B) 대기 상태. 막힌 항목과 주의사항은 `docs/dev/community.md` 상단 "재개 지점" 참고 |
...
~~4. 클라이언트가 `Authorization` 헤더를 싣지 않는다~~ — 해소(2026-08-20, E의 쿠키 인증 전환). `lib/auth.ts:45`가 `access_token` **쿠키**를 읽는다(같은 파일 주석: 문서 내비게이션에는 `Authorization` 헤더를 실을 수 없다). 같은 출처 `fetch`는 쿠키를 자동으로 보내므로 D의 커뮤니티·챗봇 호출부에 헤더를 추가할 필요가 없어졌다 — D가 보고했던 항목이라 D가 확인해 닫는다
