import type { GalleryType } from "@prisma/client"

/**
 * ── 현재 상태 (2026-08-27) ───────────────────────────────────────────────────
 *
 * **2026-08-27부터 문구는 이미지로 그린다(`_lib/banners.ts`).
 * 이 파일은 문구 원본 보관용이며 화면에서 호출하지 않는다.**
 *
 * 지우지 않는 이유: 아래 20개 문장이 배너 이미지를 만들 때의 출처다. 파일을 지우면
 * 다음 이미지를 만들 때 문구를 처음부터 다시 쓰게 되고, 그러면 아래 톤 규칙도 함께 사라진다.
 * 이미지 대신 다시 텍스트로 돌아가기로 하면 pickHopeMessage()를 그대로 부르면 된다.
 * ─────────────────────────────────────────────────────────────────────────────
 *
 * 커뮤니티 메인 희망 문구. SPEC.md 9절: 상수로 하드코딩하고 주 1회 단위로만 교체한다.
 * 문구는 한 줄짜리 문장이며 서브 문구는 두지 않는다.
 *
 * ## 문구를 고칠 때의 톤 규칙
 * 배너는 "희망 문구"다(SPEC.md 9절). 챗봇처럼 감정을 받아주는 자리가 아니라
 * 앞을 보게 하는 자리다 — 수용형("~해도 괜찮아요")보다 나아가는 문장을 쓴다.
 * 예외: 이 공간에서 무엇을 해도 된다는 안내(예: ALL의 "여기선 아무 말이나 해도 괜찮아요.")는
 * 허용한다. 금지하는 것은 무기력한 상태 자체를 그대로 두라는 수용이지, 글쓰기 문턱을
 * 낮추는 안내가 아니다.
 * 유형명을 절대 쓰지 않는다 / 조언("~해보세요") 금지 / 증상에 이름 붙이지 않는다 /
 * 과장된 칭찬 금지. `app/chat/_lib/systemPrompt.ts`의 PERSONA와 같은 기준이다.
 */
export const HOPE_MESSAGES: Record<GalleryType, readonly string[]> = {
  ALL: [
    "오늘 하루도 여기 있어줘서 고마워요.",
    "여기선 아무 말이나 해도 괜찮아요.",
    "오늘 쓴 한 줄이 누군가에겐 힘이 돼요.",
    "여기 오는 것만으로 하루가 조금 달라져요.",
    "서로 다른 하루가 여기 모여 있어요.",
  ],
  HEALTH_EMOTION: [
    "작은 시작도 분명한 시작이에요.",
    "천천히 가도 분명 가고 있어요.",
    "오늘 하나만 해냈어도 충분한 하루예요.",
    "조금씩 쌓인 하루가 내일을 만들어요.",
    "어제보다 조금 가벼워질 날이 와요.",
  ],
  INDEPENDENT_LOW_INCOME: [
    "혼자서도 여기까지 잘 왔어요.",
    "오늘 챙긴 한 끼가 내일의 힘이 돼요.",
    "조용한 하루에도 좋은 게 하나쯤 있어요.",
    "여기 들르는 것도 나를 챙기는 시간이에요.",
    "혼자여도 이 공간엔 늘 누군가 있어요.",
  ],
  FAMILY_LIVING: [
    "나만의 시간이 마음을 채워줘요.",
    "말하지 못한 마음도 여기선 꺼낼 수 있어요.",
    "여기서만큼은 나답게 있어도 돼요.",
    "오늘 삼킨 말이 여기선 이야기가 돼요.",
    "혼자 있는 시간이 나를 지켜줘요.",
  ],
}

const WEEK_MS = 604_800_000

/**
 * 이번 주 해당 갤러리의 문구를 고른다.
 *
 * epoch(1970-01-01, 목요일)부터 흐른 주 수로 나누므로 교체 시점은 매주 목요일 09:00 KST
 * (목요일 00:00 UTC)다. 요일을 옮기려면 나누기 전에 오프셋을 더해야 한다.
 *
 * 주차는 갤러리와 무관하게 한 번만 계산한다. 지금은 네 배열의 길이가 전부 5라
 * 네 갤러리의 문구가 같은 주에 함께 넘어간다. 어느 한 배열의 길이를 바꾸면
 * 그 갤러리만 다른 주기로 돌게 되므로, 길이는 넷을 같이 움직인다.
 *
 * Math.random()을 쓰지 않는 이유: 서버 렌더마다 문구가 달라져 새로고침할 때마다 바뀐다.
 * 시간·분 단위 교체도 하지 않는다 — 자주 바뀌면 사용자가 피로해진다(SPEC.md 9절).
 */
export function pickHopeMessage(gallery: GalleryType, now: Date = new Date()): string {
  const week = Math.floor(now.getTime() / WEEK_MS)
  const messages = HOPE_MESSAGES[gallery]
  return messages[week % messages.length]
}
