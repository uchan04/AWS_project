...
export function PostCard({
  ...
  return (
    ...
    >
      ...
      {post.imageUrl ? (
        <div className="min-h-0 w-full flex-1 overflow-hidden rounded-xl bg-paper-2">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={post.imageUrl} alt="" className="h-full w-full object-cover" />
        </div>
      ) : (
        <p className="min-h-0 flex-1 overflow-hidden text-sm leading-relaxed break-words whitespace-pre-line text-ink-2">{post.body}</p>
      )}
