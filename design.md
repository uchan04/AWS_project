...
- App pages(홈): **Index-First** — 홈은 카드와 링크의 목록이다. 히어로를 두지 않는다.

**판(캔버스)과 카드.** 진단·결과·시작 화면은 `#EDE5D0` 판 위에 카드가 뜬다(`.hm--canvas` + `.hm-card`).
Figma 프로토타입의 구성을 그대로 옮긴 것이다. 홈만 종이색 배경을 그대로 쓴다.

**폭은 화면이 정한다**(`--col-max`). 진단 42.5rem · 결과·홈 52.5rem · 시작 56.25rem.
넓은 화면에서는 두 열, 좁은 화면에서는 자동으로 한 열이 된다(`repeat(auto-fit, minmax(min(Npx,100%),1fr))`).
격자를 감싼 flex 부모에는 `min-width: 0`을 준다. 없으면 격자가 부모를 밀어낸다.

화면 전체를 덮는 배경 이미지 히어로는 쓰지 않는다.

**진행률 바.** 값은 "유형이 좁혀진 정도"다. 총 문항 수는 어디에도 노출하지 않는다 —
조기 종료 때문에 사람마다 다르다(`SPEC.md` 3절). Figma의 `질문 n / 6` 카운터는 가져오지 않았다.
...
- 이 앱이 쓰는 모션은 **다섯 개뿐**이다. 앞의 셋은 Figma의 `fadeSlideIn`·`float`·`bounceIn`을
  그대로 옮겼다. 등장(`.hm-fade` = opacity + `translateY(10px)`), 마스코트(`.hm-float` = 3s 상하 8px),
  결과 마스코트 등장(`.hm-bounce` = scale 0.4 → 1.08 → 0.96 → 1, 500ms),
  누름(`translateY(1px)`), 카드 hover(그림자 + `translateY(-1px)`).
- 스크롤 연동 애니메이션과 등장 스태거는 쓰지 않는다. 확대(scale)는 결과 마스코트 1회에만 쓴다.
...
- 색 면적: 한 화면에 종족 원판 1개 + 희석된 면 1개까지. 그 이상 채우지 않는다.
  진단 전 시작 화면만 예외로 세 종족 타일을 나란히 둔다 — 아직 내 종족이 없어서 강조할 색도 없다.
...
- 카드(`.hm-card`) 하나가 모든 화면의 면을 담당한다. 카드 안의 작은 항목은 타일(`.hm-tile`)이다
- 왼쪽 정렬. 가운데 정렬은 진단 질문과 종족판 안에서만 쓴다
